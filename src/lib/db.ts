...
const INITIAL_POSTS: Post[] = [
  ...
  { id: "about", title: "About June", subtitle: "사유하고 창작하는 삶에 대하여", content: "# JUNE\n경계선 없는 편집, 글쓰기 앱\n\n소형 기기 화면에서 프레임의 크기에 얽매이지 않고 문자나 이미지의 위치를 자유롭게 조절하고 편집할 수 있도록 설계했습니다. 오직 타이포그래피에만 시선이 쏠리도록 여백을 두고, 본문을 세심하게 정돈했습니다. 글을 쓰는 동안에는 오롯이 글과 하나되는 경험을 하도록 도와주며, 소소한 생각의 층위들이 한 장의 기록으로 남는 과정에 집중하도록 디자인되었습니다. 글과 인생이 하나가 되는 순간, 그 차분한 동행을 꿈꿉니다. 이 공간은 생각을 정돈하고 기억을 갈무리하는 집이며, 오직 글자 자체와 호흡하는 침묵의 대화 공간입니다.\n\n― motto\n모든 생각을 묶어두고 생각의 이면의 사유에 매료되는 삶.\n모든 것을 기록하기 위해 다만 펜을 들뿐.\n모든 이의 마음 깊은 곳에 가닿는 글을 씁니다.\nJune Kim. 2026\n\n# WORKS\n생각을 설계하고 구현하기\n\n기능의 연구와 디자인 기획에서부터 실제 코드의 구현에 이르기까지, 모든 미술적 사유와 기술적 완성이 조화를 이루는 개발을 추구합니다. 사용자와 상호작용하는 단순하고 우아한 인터페이스를 구축하고, 물성을 가진 아날로그 오거나이저처럼 정갈한 시스템을 만듭니다. 이번 작업들은 그러한 생각의 조각들을 하나하나 실체로 빚어낸 흔적들입니다.\n\n― 모바일용 디자인\nOne-touch UI (100% 한 손 조작 가능하도록 하단 레이아웃 배치)\n스크롤 반응형 상하단 바 (스크롤에 조화롭게 연계되는 타이틀 바)\n반응형 그리드 시스템 (모든 해상도 지원)\n가독성을 우선한 나눔명조 폰트 지정\n\n― 퍼블리싱 가이드\nGithub Pages 기반의 정적 페이지 빌드\nVercel과 Next.js를 통한 고품질 배포\nTailwind CSS를 통한 빠르고 일관된 스타일링\nSEO 메타 태그의 정밀한 구성\n\n― 주요 작업\n블로그 CMS 및 글 관리 시스템 (본 프로젝트)\n아날로그 원목 오거나이저 디자인 및 제작\n한글 타이포그래피 대칭/비대칭 분석 포스터\n가상 갤러리 3D 인터랙션 공간 브랜딩\n\n― 도서/문헌\n'기록의 온도' (단단한 사유를 위한 일기 가이드)\n'미니멀리스트의 공간' (여백과 가독성에 관하여)\n'자바스크립트의 숨겨진 철학' (구조적 논리 분석)\n\n― 수상\n코리아 디자인 어워드 미디어 부문 대상 (2025)\n대한민국 소프트웨어 어워드 우수상 (2025)\n\n# JUNEWORKS\n사유를 흐르게 하는 공간\n\nJUNEWORKS는 창작자 June(김준)이 직접 경험하고 사유한 생각의 기록들을 세상과 공유하고, 세상과 대화하기 위해 마련한 독립적인 스튜디오 공간이자 브랜드입니다. 이곳에서 제안하는 작업들과 글들이 지친 이들에게 고요한 휴식을 선사하고, 삶의 작은 울림이 되기를 바랍니다. since 2022", createdAt: new Date("2026-06-01T09:00:00Z").toISOString(), type: "post" }
];
...
const getLocalPosts = (): Post[] => {
  ...
  try {
    ...
    const needsMigration = parsed.length < INITIAL_POSTS.length || !targetPost || !targetPost.content.startsWith("엄마,") || !hasWorkPosts || !hasAboutPost;
    if (needsMigration) {
      localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(INITIAL_POSTS));
      return INITIAL_POSTS;
    }
